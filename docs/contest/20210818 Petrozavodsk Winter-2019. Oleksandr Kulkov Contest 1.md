...
首先考虑所有字符均已确定的情况。若$p_i$​的值可以为$j$​，则$\forall x\in[1,m),s[x][i]=s[x+1][j]$​均成立可以看作是两个长度为$m-1$​的字符串$s_1[i],s_2[j]$​能匹配，其中$s_1[i][x]=s[x][i],s_2[j][x]=s[x+1][j]$​​​。这样利用字符串hash就可以快速得出匹配情况。对于$s_2[i]$​，假设有$a[i]$​个$s_1$​能与其匹配，则答案为$\Pi_{i=1}^na[i]!$​，在这里，$0!=0$​。用map统计一下字符串的出现次数就好了，复杂度$\mathcal{O}(nm)$​。
